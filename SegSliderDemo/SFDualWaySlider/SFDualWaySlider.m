...
- (void)initSliderUI{
    ...
    UITapGestureRecognizer *progressViewPan = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(progressViewIsClicked:)];
    progressViewPan.delegate = self;
    [_progressView addGestureRecognizer:progressViewPan];
    ...
    // zyz为了上层高亮的圆角_lightView加到_progressView上
    [_progressView addSubview:_lightView];
...
- (void)panMinAction:(UIPanGestureRecognizer *)pan{
    ...
    switch (pan.state) {
        ...
        case UIGestureRecognizerStateChanged:{
            ...
            if (point.x <= _progressLeftSpace + _minRange) {
                x = 0.f + _minRange;
...
- (void)panMaxAction:(UIPanGestureRecognizer *)pan{
    ...
    switch (pan.state) {
        ...
        case UIGestureRecognizerStateChanged:{
            ...
            if (point.x >=  (_progressWidth + _progressLeftSpace) - _maxRange) {
                y = _maxRange;
...
/**
 滑杆是否能编辑
 */
- (void) canEdit:(BOOL)isCan
{
    if (isCan)
    {
        self.lightView.hidden = NO;                         // 显示 正在选择
        self.minIndicateView.hidden = NO;                   // 显示 左侧选择块
        self.maxIndicateView.hidden = NO;                   // 显示 右侧选择块
    }
    else
    {
        self.lightView.hidden = YES;                        // 隐藏 正在选择
        self.minIndicateView.hidden = YES;                  // 隐藏 左侧选择块
        self.maxIndicateView.hidden = YES;                  // 隐藏 右侧选择块
    }
}


- (void)progressViewIsClicked:(UIPanGestureRecognizer *)pan
{
    CGPoint point = [pan locationInView:_progressView];
    if (self.clickProgressView)
    {
        self.clickProgressView(point.x);
    }
